            ...
            <div class="container mx-auto">
                <?php foreach ($orders_array as $orders) { ?>
                    <div class="grid grid-cols-4 gap-6 mt-10">
                        <div class="flex flex-col justify-center p-6 text-white bg-gray-500 border-2 border-gray-300 cursor-pointer rounded-xl">
                            <div class="my-5">
                            ...
                            <?php if ($orders['OrderStatus'] == 'Ready') : ?>
                                <!-- Update order status button  -->
                                <div class="my-5">
                                    <button id="btn" onclick="ShowModal('myModal-<?= $orders['OrderID'] ?>')" class="px-8 py-3 text-base font-semibold text-white bg-indigo-500 rounded-md outline-none hover:shadow-form">
                                        Update order status
                                    </button>
                                </div>
                            <?php endif; ?>
                            <div id="myModal-<?= $orders['OrderID']; ?>" class="hidden modal">
                                <form method="post" action="update_order.php" role="form" enctype="multipart/form-data">
                                    ...
                                    <label for="orders" class="block mb-2 text-sm font-medium text-white">Choose order
                                        status</label>
                                    <select id="orders" name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                        <option value="Cancelled">Cancelled</option>
                                        <option value="Shipped">Shipped</option>
                                    </select>
                                    <button type="submit" name="submit" class="px-2 py-4 mt-4 text-base font-semibold text-white bg-indigo-500 rounded-md outline-none hover:shadow-form">Confirm</button>
                                </form>
                            </div>
                        </div>
                    </div>
